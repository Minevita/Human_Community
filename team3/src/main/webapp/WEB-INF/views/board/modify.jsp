                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
							<c:choose>
								<c:when test="${board.category eq 1}"> <img src="${pageContext.request.contextPath}/resources/img/bullhorn_solid.svg" width="50"> 공지 수정 </c:when>
								<c:when test="${board.category eq 2}"> <img src="${pageContext.request.contextPath}/resources/img/comment_regular.svg" width="50"> 포럼 수정 </c:when>
								<c:when test="${board.category eq 3}"> <img src="${pageContext.request.contextPath}/resources/img/edit_regular.svg" width="50"> 서적리뷰 수정 </c:when>
								<c:when test="${board.category eq 4}"> <img src="${pageContext.request.contextPath}/resources/img/comment_regular.svg" width="50"> 자유게시판 수정 </c:when>
								<c:when test="${board.category eq 5}"> <img src="${pageContext.request.contextPath}/resources/img/newspaper_regular.svg" width="50"> IT News 수정 </c:when>
								<c:when test="${board.category eq 6}"> <img src="${pageContext.request.contextPath}/resources/img/desktop_solid.svg" width="50"> IT 정보 수정 </c:when>
								<c:when test="${board.category eq 7}"> <img src="${pageContext.request.contextPath}/resources/img/store_solid.svg" width="50"> 중고장터 수정 </c:when>
                        ...
                        <div class="card-body">
                        	<form method="post">
                        		<div class="form-group">
		                       		<p class="form-control">${board.nickname}</p>
			                        <p class="form-control">${board.regDate} 작성 ${board.updateDate} 수정됨</p>
	                           </div>
                        		<div class="form-group">
                        			<p># ${board.bno} <img class="img-control" src="${pageContext.request.contextPath}/resources/img/comment_alt_regular.svg" width="15">${board.replyCnt}</p>
	                           ...
	                           <div class="form-group">
		                           ...
		                           <input type="hidden" id="nickname" name="nickname" value="${board.nickname}">
	                           </div>
	                           <div class="form-group">
		                           <input type="hidden" class="form-control" id="category" name="category" value="${board.category}">
	                           </div>
	                           <c:choose>
	                           ...
	                           <sec:csrfInput/>
	                           <button class="btn btn-warning" formaction="modify" id="btnSubmit">수정</button>
	                           <button class="btn btn-danger" formaction="remove">삭제</button>
	                           </c:if>
	                           </sec:authorize>
	                           <a class="btn btn-primary" href="list?pageNum=1&amount=10&category=${page.cri.category}" id="list">목록</a>
                           </form>
                        </div>
                    </div>
                    <h1></h1>
...
// 	})
	...
	$(function() {
        ...
        $.getJSON("/board/getUMKs/"+bno).done(function(result) {
        	console.log(result);
        	makePrevMap(result);
        })
	})
